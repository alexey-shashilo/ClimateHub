...
{
    private string _connectionString = "Host=localhost;Port=5432;Database=climate_hub_recovery;Username=climate_hub;Password=climate_hub_recovery;";
    private int _mqttPort = 1883;
    private string _mqttHost = "localhost";

    public RuntimeRestartRecoveryTests()
    {
        _connectionString = Environment.GetEnvironmentVariable("RECOVERY_PG_CONNECTION")
            ?? "Host=localhost;Port=5432;Database=climate_hub_recovery;Username=climate_hub;Password=climate_hub_recovery;";
        _mqttPort = int.TryParse(Environment.GetEnvironmentVariable("RECOVERY_MQTT_PORT"), out var mp) ? mp : 1883;
    }
